import logging
from asyncio import FIRST_COMPLETED, Event, Task, create_task, sleep, wait
...
async def wait_for_new_events(
    event_fetcher: IncrementalTimerEventFetcher, timer_fetcher: RunningTimersFetcher, broker: Broker, event: Event
):
...
async def resolve_running_timers(root, info: ResolveInfo):
    ...
    event_fetcher = IncrementalTimerEventFetcher(http_request_context.db)
    ...
    async with http_request_context.db.session() as session:
        ...
        wait_for_new_events_task = create_task(
            wait_for_new_events(event_fetcher, timer_fetcher, http_request_context.broker, event)
        )
